...
console.log('API baseURL:', baseURL); // 디버깅용 로그 추가
...
axiosInstance.interceptors.response.use(
  ...
  async error => {
    ...
    if (
      ...
      typeof window !== 'undefined' // SSR 고려
    ) {
      ...
      try {
        ...
        // Refresh Token 요청 실패 시 로그아웃 처리 및 로그인 페이지로 리다이렉트
        console.error('Refresh Token Error', err);

        // 로컬 스토리지 초기화 (모든 인증 관련 데이터 제거)
