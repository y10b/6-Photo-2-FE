...
export default function FilterBottomSheet({
  ...
  // createPortal이 클라이언트 측에서만 동작하기때문에 서버 렌더링 시 발생하는 문제를 방지하기 위함
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-center items-end">
      ...
      {/* 바텀시트 */}
      <div className="relative z-10 w-[1199px] h-[480px] bg-[#1b1b1b] text-gray400 rounded-t-[16px] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 pb-10 gap-[11px]">
          <button
            onClick={() => {
              setSelectedValues([]);
              onApply({ type: selectedTab, value: "" });
            }}
          ...
          <Button
            ...
            variant="primary"
          >
            {selectedValues.length === 0 && filterCounts
              ? Object.values(filterCounts.grade || {}).reduce(
                  (sum, count) => sum + count,
                  0
                ) ||
                Object.values(filterCounts.genre || {}).reduce(
                  (sum, count) => sum + count,
                  0
                )
